defmodule EStateBoxTest do
  ...
  test "readme orderset test" do
    new = EStateBox.new(fn () -> [] end)
    childA = EStateBox.modify({&:ordsets.add_element/2, [:a]}, new)
    childB = EStateBox.modify({&:ordsets.add_element/2, [:b]}, new)
    resolved = EStateBox.merge([childA, childB])
    assert [:a, :b] == EStateBox.value(resolved)
  end

  test "readme ordsets manul test" do
    new = EStateBox.new(0, fn() -> [] end)
    childA = EStateBox.modify(1, {&:ordsets.add_element/2, [:a]}, new)
    childB = EStateBox.modify(2, {&:ordsets.add_element/2, [:b]}, new)
    resolved = EStateBox.merge([childA,childB])
    assert [:a, :b] == EStateBox.value(resolved)
  end

  test "is EStateBox test" do
    assert false == EStateBox.is_statebox(:not_a_statebox)
    assert true == EStateBox.is_statebox(EStateBox.new(fn() -> :is_a_statebox end))
  end
